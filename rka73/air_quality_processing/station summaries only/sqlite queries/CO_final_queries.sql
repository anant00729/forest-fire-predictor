...
-- add Year column
select strftime('%Y', DATE_PST) from `CO_1980_2008`;
ALTER TABLE `CO_1980_2008` ADD YEAR INT(4);
UPDATE `CO_1980_2008` SET `YEAR` = strftime('%Y', `DATE`);
SELECT `DATE`, `YEAR` from `CO_1980_2008`;

-- add Month column
select strftime('%m', DATE), DATE from `CO_1980_2008`;
ALTER TABLE `CO_1980_2008` ADD `MONTH` INT(2);
UPDATE `CO_1980_2008` SET `MONTH` = strftime('%m', `DATE`);
SELECT `DATE`, `MONTH` from `CO_1980_2008`;

-- add Date column
select strftime('%d', DATE), DATE from `CO_1980_2008`;
ALTER TABLE `CO_1980_2008` ADD `DAY` INT(2);
UPDATE `CO_1980_2008` SET `DAY` = strftime('%d', `DATE`);
SELECT `DATE`, `DAY` from `CO_1980_2008`;
...
from `CO_1980_2008`  GROUP By STATION_NAME, YEAR,MONTH;
...
from `CO_1980_2008`  GROUP By STATION_NAME, YEAR;
...
join "CO_1980_2008_stats_annual" a on m. "STATION_NAME" = a."STATION_NAME" and m. "YEAR" = a."YEAR";
--------------------------------------------------------------------------------
...
-- add Year column
select strftime('%Y', DATE_PST) from `CO_2009_2020`;
ALTER TABLE `CO_2009_2020` ADD YEAR INT(4);
UPDATE `CO_2009_2020` SET `YEAR` = strftime('%Y', `DATE`);
SELECT `DATE`, `YEAR` from `CO_2009_2020`;

-- add Month column
select strftime('%m', DATE), DATE from `CO_2009_2020`;
ALTER TABLE `CO_2009_2020` ADD `MONTH` INT(2);
UPDATE `CO_2009_2020` SET `MONTH` = strftime('%m', `DATE`);
SELECT `DATE`, `MONTH` from `CO_2009_2020`;

-- add Date column
select strftime('%d', DATE), DATE from `CO_2009_2020`;
ALTER TABLE `CO_2009_2020` ADD `DAY` INT(2);
UPDATE `CO_2009_2020` SET `DAY` = strftime('%d', `DATE`);
SELECT `DATE`, `DAY` from `CO_2009_2020`;
...
SELECT NAPS_ID, EMS_ID, STATION_NAME, REGION, OWNER, DATE_PST, DATE, YEAR, MONTH,
round(avg(ROUNDED_VALUE), 3) as `MONTHLY1-HR_AVG`
from `CO_2009_2020`  GROUP By STATION_NAME, YEAR,MONTH;

-- create annual stats table like above but with a column containing ANNUAL 1 hr averages
CREATE TABLE `CO_2009_2020_stats_annual` AS
SELECT NAPS_ID, EMS_ID as EMSID, STATION_NAME, REGION, OWNER, DATE_PST, DATE, YEAR,
round(avg(ROUNDED_VALUE), 3) as `ANNUAL1-HR_AVG`
from `CO_2009_2020`  GROUP By STATION_NAME, YEAR;
...
join "CO_2009_2020_stats_annual" a on m. "STATION_NAME" = a."STATION_NAME" and m. "YEAR" = a."YEAR";
